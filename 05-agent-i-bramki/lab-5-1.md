...
**Tag startowy: `lab-5-1-start`** · **Produkt: `.claude/settings.json`, `.claude/hooks/*.sh`, cel `gate` w `Makefile`**
...
i zobaczyć różnicę w działaniu.

---

## Krok 1 - pokaz różnicy: prośba kontra egzekucja
...
Sprawdzenie, jak działa **jako prośba**:
...
**Do notatek:** czy model dopisał test i czy chociaż wspomniał, że go nie ma.

Powtórzenie tego samego kilka razy albo porównanie z sąsiadem: **wynik bywa różny** -
i to jest cała istota kroku. Prośba działa w większości przypadków. W większości, nie zawsze.
...
> Uwaga: `gate` to **na razie** lint i testy. Skan sekretów dochodzi w module 8 -
> gdyby wszedł teraz, znalazłby coś, czego ten moduł jeszcze nie obejmuje.
>
...
**Test bez Claude Code** - hook to zwykły skrypt, da się go sprawdzić wprost:
...
Do sprawdzenia także przypadki brzegowe - `.md` zamiast `.py`, plik, którego nie ma.
...
Wzorzec blokujący każde `rm` daje hook, który przeżyje jeden dzień.

---

## Krok 5 - bramka na `Stop`

To jest hook, który realnie zmienia sposób pracy. I to jest hook, przy którym najłatwiej
napisać obejście problemu rozwiązanego już w samym narzędziu.

**Schemat wejścia trzeba przeczytać przed napisaniem promptu.** Zdarzenie `Stop` dostaje na stdin
...
worktree, w którym faktycznie pracuje sesja. Pokazuje to lab 6.1.

### Dlaczego to jest ważniejsze niż sam hook

Odruch przy tym zadaniu brzmi: „trzeba jakoś zapamiętać, że blokada już była" - i powstaje
plik-znacznik w `scratchpad_dir`, kluczowany po `session_id`. To działa. Jest dwa razy
dłuższe, wymaga obsługi nieobecnego pola `scratchpad_dir` i wymusza kasowanie
...
> **Schemat wejścia trzeba przeczytać, zanim powstanie obejście.** To jest ta sama zasada,
> co żądanie cytatu z modułu 1: sprawdzić, co jest, zamiast zakładać, czego nie ma.
...
## Krok 6 - sprawdzenie na żywo

```
/hooks
```

Wszystkie trzy mają być widoczne pod swoimi zdarzeniami. Potem powtórzenie kroku 1:
...
- [ ] Bramka używa `.cwd`, nie `${CLAUDE_PROJECT_DIR}`, a powód tego wyboru jest jasny.
- [ ] Blokada zadziałała na żywo: model nie mógł zakończyć tury przy czerwonym lincie.
- [ ] `.claude/settings.json` jest **zacommitowany** - to standard zespołu, nie ustawienie prywatne.

## Pułapki

**Hook w `~/.claude/settings.json` zamiast w repo.** Wtedy to jest prywatna wygoda,
...
i wymusza kasowanie przy zielonej bramce. Schemat wejścia wypada przejrzeć,
...
Formatowaniu podlega wyłącznie `.tool_input.file_path`.

**`exit 1` zamiast `exit 2`.** Kod 1 to błąd nieblokujący: akcja przechodzi, a w transkrypcie
zostaje tylko notka. Blokuje wyłącznie **2**.
