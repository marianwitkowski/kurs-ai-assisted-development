...
**Czas: ~40 min** · **Tag startowy: `lab-8-1-start`** · **Produkt: poprawki w `app/`, `tests/test_bezpieczenstwo.py`, skan sekretów w `make gate`**
...
Uruchomić na tym repozytorium workflow, który zbudowałeś wczoraj, znaleźć trzy klasy luk,
naprawić je i **zabezpieczyć przed powrotem**.

---

## Krok 1 - przegląd (8 min)
...
> bo nikt ich nie wprowadził; one tam po prostu były. Sprawdź sam:
>
> ```bash
...
> zanim jeszcze zobaczysz raport.

Skill przejdzie cały projekt. Przeczytaj raport i sprawdź:
...
Powinien znaleźć trzy klasy problemów. **Jeśli znalazł mniej - zanim przejdziesz dalej,
popraw skill, nie kod.** To jest ten moment, w którym artefakt zespołowy zarabia na siebie.

---

## Krok 2 - potwierdzenie luk (7 min)

Raport to hipoteza. Sprawdź ją.
...
Zapisz, ile **dokładnie** endpointów ma problem z autoryzacją. Raport mógł wskazać jeden -
sprawdź wszystkie.

---

## Krok 3 - naprawa (12 min)
...
Po zakończeniu - **przeczytaj diff**:

```bash
git diff
```

Sprawdź, czy agent poprawił wszystkie cztery endpointy z problemem autoryzacji,
czy tylko ten, który był w raporcie.

---

## Krok 4 - testy regresyjne (5 min)
...
Sprawdź, że działa - wprowadź sekret z powrotem:
...
> Dopisujemy linię na końcu pliku zamiast podmieniać istniejącą przez `sed` - twoja poprawka
> z kroku 3 mogła wyglądać inaczej niż wzorcowa i wzorzec `sed` by nie trafił. Test skanera
> ma sprawdzać **skaner**, a nie to, czy zgadłeś kształt cudzej poprawki.
...
## Krok 6 - to, co zostaje w historii (5 min)

Klucz jest usunięty z pliku. Sprawdź, czy jest usunięty z repozytorium:
...
- [ ] Potwierdziłeś każdą lukę **wykonaniem**, nie tylko czytaniem raportu.
- [ ] Naprawione są **wszystkie cztery** endpointy z problemem autoryzacji.
- [ ] Cudzy zasób daje 404, i wiesz dlaczego nie 403.
...
- [ ] Wiesz, że klucz nadal jest w historii gita, i wiesz, co z tym zrobić.
